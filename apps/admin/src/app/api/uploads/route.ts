...
export async function POST(request: Request) {
  ...
  if (!UploadService.isConfigured()) {
    return NextResponse.json(
      {
        error:
          'Cloudinary is not configured on this server. Add CLOUDINARY_CLOUD_NAME, CLOUDINARY_API_KEY, and CLOUDINARY_API_SECRET in Vercel → admin project → Settings → Environment Variables, then redeploy.',
      },
      { status: 503 },
    )
  }
  ...
  if (file.size > MAX_BYTES) {
    return NextResponse.json(
      { error: 'File is too large (max 4MB). Compress the image or use a smaller file.' },
      { status: 400 },
    )
  }
