  ...
  if (items.length === 0) {
    return (
      <main className="mx-auto flex min-h-[50vh] max-w-3xl flex-col items-center justify-center px-4 py-16 text-center">
        ...
        <p className="mt-2 text-[#514534]">Browse trending products and add favourites to get started.</p>
            ...
            <Card className="border-[#d6c4ad]" key={group.merchantId}>
              ...
              <CardContent className="space-y-4">
                {group.items.map((item) => (
                  <div className="flex gap-4 border-t border-[#eee7de] pt-4 first:border-0 first:pt-0" key={item.productId}>
                    <div className="relative h-24 w-24 shrink-0 overflow-hidden rounded-lg bg-[#f9f3ea]">
                      {item.imageUrl ? (
                        <Image alt={item.name} className="object-cover" fill sizes="96px" src={item.imageUrl} unoptimized />
                      ) : null}
                    </div>
                    <div className="min-w-0 flex-1">
                      <Link className="font-semibold hover:text-[#7f5700]" href={`/products/${item.slug}`}>
                        {item.name}
                      </Link>
          ...
          <Card className="sticky top-24 border-[#d6c4ad]">
            ...
            <CardContent className="space-y-4">
              ...
              <div className="flex justify-between text-lg font-bold">
                <span>Total</span>
                <span className="text-[#a83635]">{priceFormatter.format(grandTotalInPence / 100)}</span>
              </div>
